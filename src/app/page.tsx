  ...
  const steps = [
    {
      ...
      desc: "Inscrivez-vous gratuitement sur 26KADO en quelques secondes.",
    ...
    },
    {
      ...
      desc: "Connectez-vous à votre espace 26KADO et téléchargez vos captures.",
    ...
    },
    {
      ...
      desc: "Chaque inscription validée vous rapporte 25% du montant déposé.",
      color: "from-green-500 to-green-600",
    },
    {
      icon: Clock,
      title: "5. Validation sous 24 à 48h",
      desc: "Notre équipe vérifie vos preuves sous 24 à 48h.",
      color: "from-yellow-500 to-yellow-600",
    },
    {
      icon: CreditCard,
      title: "6. Recevoir vos gains",
      desc: "Les fonds sont crédités sur votre wallet, retirez à tout moment.",
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <Card variant="premium" className="p-8 lg:p-10">
        ...
        <div className="mb-10">
          <h2 className="text-3xl lg:text-4xl font-bold text-white text-center mb-2">Comment gagner de l'argent ?</h2>
          <p className="text-gray-400 text-center mb-8">Suivez ce guide simple en 6 étapes</p>
          <div className="space-y-6">
            {steps.map((step, i) => {
              const Icon = step.icon;
              return (
                <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="flex items-start gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${step.color} flex items-center justify-center shrink-0`}><Icon className="w-6 h-6 text-white" /></div>
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-white mb-1">{step.title}</h3>
                    <p className="text-sm text-gray-400">{step.desc}</p>
                    {step.link && <Link href={step.link}><Button variant="premium" size="sm" className="mt-2">{step.linkText} <ArrowRight className="ml-1 w-3 h-3" /></Button></Link>}
                  </div>
                </motion.div>
              );
            })}
        ...
        <div>
          <h3 className="text-2xl font-bold text-white text-center mb-6"><Calculator className="w-6 h-6 inline mr-2 text-blue-400" /> Simulez vos gains</h3>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-4xl mx-auto">
            <Card className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm text-gray-300">Nombre de personnes invitées : <span className="text-blue-400 font-bold">{simPeople}</span></label>
                <input type="range" min="1" max="100" value={simPeople} onChange={(e) => setSimPeople(Number(e.target.value))} className="w-full h-2 rounded-full appearance-none bg-white/10 cursor-pointer accent-blue-500" />
                <div className="flex justify-between text-xs text-gray-500"><span>1</span><span>50</span><span>100</span></div>
              </div>
              <div className="space-y-2">
                <label className="text-sm text-gray-300">Montant moyen du dépôt : <span className="text-blue-400 font-bold">{formatCurrency(simAmount)}</span></label>
                <input type="range" min="5000" max="200000" step="5000" value={simAmount} onChange={(e) => setSimAmount(Number(e.target.value))} className="w-full h-2 rounded-full appearance-none bg-white/10 cursor-pointer accent-blue-500" />
                <div className="flex justify-between text-xs text-gray-500"><span>5 000</span><span>100 000</span><span>200 000</span></div>
              </div>
            </Card>
            <Card variant="premium" className="space-y-4">
              <h3 className="text-lg font-semibold text-white mb-4">Vos gains estimés</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-white/5 text-center"><p className="text-xs text-gray-400 mb-1">Commission</p><p className="text-2xl font-bold text-green-400">25%</p></div>
                <div className="p-4 rounded-xl bg-white/5 text-center"><p className="text-xs text-gray-400 mb-1">Bonus parrainage</p><p className="text-2xl font-bold text-purple-400">{formatCurrency(bonus)}</p></div>
              </div>
            </Card>
          </div>
        </div>
    ...
    <main className="min-h-screen">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden pt-16">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <h1 className="text-5xl sm:text-6xl lg:text-8xl font-bold mb-4"><span className="text-gradient glow-blue">26KADO</span></h1>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12 max-w-4xl mx-auto">
              {[
                ...
                <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.1 }} className="glass-card rounded-2xl p-4">
                  <div className="text-2xl lg:text-3xl font-bold text-gradient"><CountUp end={stat.value} />{stat.suffix}</div>
      ...
      <Section>
        <div className="text-center mb-12">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">Des avantages exclusifs pour les membres de la communauté 26KADO.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {siteConfig.advantages.map((adv, i) => {
            const IconComponent = { Gift, Headphones, Ticket, Users, TrendingUp, Zap }[adv.icon] || Gift;
            return (
              <motion.div key={adv.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                <Card variant="interactive" className="h-full">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center mb-4"><IconComponent className="w-6 h-6 text-blue-400" /></div>
      ...
      <Section>
        <Card variant="premium" className="text-center py-12 px-8 max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">Recevez les offres exclusives</h2>
            <p className="text-gray-400 mb-8 max-w-2xl mx-auto">Rejoignez notre groupe WhatsApp et recevez en avant-première toutes les offres et codes promo.</p>
            <a href="https://whatsapp.com/channel/0029Vb7OtjLBqbr4XtpvJx0n" target="_blank" rel="noopener noreferrer">
      ...
      <Section className="text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="text-3xl lg:text-5xl font-bold text-white mb-4">Prêt à commencer ?</h2>
          <p className="text-gray-400 mb-8 max-w-xl mx-auto">Rejoignez des milliers de membres et profitez des meilleures offres.</p>
          {user ? (
            <Link href="/dashboard"><Button size="xl" variant="premium">Accéder à mon espace <ArrowRight className="ml-2 w-5 h-5" /></Button></Link>
          ) : (
            <Link href="/auth/register"><Button size="xl" variant="premium">Créer mon compte gratuitement <ArrowRight className="ml-2 w-5 h-5" /></Button></Link>
          )}
        </motion.div>
      </Section>
